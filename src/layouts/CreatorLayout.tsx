...
export function CreatorLayout({ page }: { page: LandingPage }) {
  return (
    <>
      <SeoBlock page={page} />
      <div className="min-h-screen bg-zinc-950">
        <div className="relative bg-gradient-to-b from-zinc-900/50 to-zinc-950">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-8">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {['Создавай', 'Экспериментируй', 'Презентуй'].map((motto) => (
              <div key={motto} className="card text-center py-6 border-t-2 border-t-emerald-500">
                <div className="text-sm sm:text-base font-semibold text-emerald-400">{motto}</div>
              </div>
            ))}
          </div>
        </div>
